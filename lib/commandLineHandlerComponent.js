...
module.exports = function registerComponent (cmdConfig) {
    ...
    var CommandLineHandler,
        ...
        commandLineCategory = 'command-line-handler',
        entry = 'm-' + commandLineName,
    ...
    Factory({ // unless unregister:false, will auto-unregister on unload
        contract: contractId,
    //  id: '{7f397cba-7a9a-4a05-9ca7-a5b8d7438c6c}', // doesn't working with both an ID and contract ID
        Component: CommandLineHandler
    });

    catMan.addCategoryEntry(commandLineCategory, entry, contractId, false, true);
    ...
    */
    return {
        unregister: function () {
            catMan.deleteCategoryEntry(commandLineCategory, entry, false);
        }
    };
};
